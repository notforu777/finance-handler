...
object Errors {
  ...
  @derive(encoder, decoder)
  case class ExpAlreadyExists()
    extends AppError("Exp with same name already exists")

  @derive(encoder, decoder)
  case class ExpNotFound(id: ExpId)
    extends AppError(s"Exp with id ${id.value} not found")
